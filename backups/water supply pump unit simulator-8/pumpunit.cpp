...
double PumpUnit::CalcuSystemFlow(double *balanced_pump_head)
{
    ...
    double flow_low = 0;
    double flow_middle = m_ro_data.flow_total;
    ...
    *balanced_pump_head = 0;
    ...
    do
    {
        double pump_head = GetPumpHeadFromFlow(m_ro_data.flow_total);
        double valve_dp = GetValveDpFromFlow(m_ro_data.flow_total);
        if(valve_dp > 9000)
        {
            balanced_flow = 0;
            *balanced_pump_head = GetPumpHeadFromFlow(0);
            break;
        }

        if(qAbs(pump_head - valve_dp) < (pump_head * 0.001))
        {
...
void PumpUnit::Run_cycle(int period_ms)
{
    ...
    //m_ro_data.pressure_at_use_point =
    ...
    //assign dp to valves
    m_valves[2].SetDifferentialPressure(pump_head/10+m_ro_data.pressure_at_inlet);
    m_valves[1].SetDifferentialPressure(pump_head/10+m_ro_data.pressure_at_inlet-m_system_profile.pressure_static);
    m_valves[0].SetDifferentialPressure(pump_head/10+m_ro_data.pressure_at_inlet-m_system_profile.pressure_static);
...
Q1=head^1/2*kv1
********************************************************************************/
double PumpUnit::GetValveDpFromFlow(double flow)
{
    if(m_valves[0].ReadRoData().Kv < 0.1 && m_valves[1].ReadRoData().Kv < 0.1 && m_valves[2].ReadRoData().Kv < 0.1)
        return 9999;
    ...
    //maximum pressure is pump's head

    double low_dp = m_system_profile.pressure_static;
    double high_dp = m_system_profile.pressure_at_outlet - m_system_profile.pressure_at_inlet;
    double middle_dp = (high_dp-low_dp)/2;
    double balanced_dp=0;
    int cycles = 0;
    do {
        //flow of valve 1
        //dp1 = total dp;
        //flow 1 = dp1^1/2 * Kv1
        double dp_1 = middle_dp;
        double flow_1 = pow(dp_1,0.5) * m_valves[0].ReadRoData().Kv;

        //flow of valve 2
        //pressure loss 1 = (flow 2 + flow 3)^2 * K
        //dp2 = dp1 - static pressure/2 - pressure loss 1
        //flow 2 = dp2^1/2 * Kv2
        double ploss_1 = pow((flow-flow_1),2) * K;
        double dp2 = dp_1 - m_system_profile.pressure_static / 2 - ploss_1;
        double flow_2 = pow(dp2,0.5) * m_valves[1].ReadRoData().Kv;

        //flow of pump 3
        //pressure loss 2 = (flow 3)^2 * K
        //dp3 = dp2 - static pressure /2 - pressure loss 2
        //flow 3 = dp3^1/2 * Kv3
        double ploss_2 = pow((flow - flow_1 - flow_2),2) * K;
        double dp3 = dp2 - m_system_profile.pressure_static / 2 -ploss_2;
        double flow_3 = pow(dp3,0.5) * m_valves[2].ReadRoData().Kv;

        //check total flow
        double calcu_flow=flow_1 + flow_2 + flow_3;

        if (qAbs(calcu_flow - flow) < (0.01 * flow))
        {
            balanced_dp = middle_dp;
            break;
        }
        else
            if (calcu_flow < flow)
