...
Pump_db::Pump_db()
{
    ...
    m_tableName.loadprofileTableName=configIniRead->value("/table_name/loadprofileTableName").toString();
...
void Pump_db::setTableName(DbTableName tableName)
{
    ...
    configIniWrite->setValue("table_name/loadprofileTableName", m_tableName.loadprofileTableName);
...
bool Pump_db::getPumpData(pumpSelection pumpSelection,Pump::PerformData *pumpPerformanceData)
{
    ...
    if(query.size()<1)
    {
        QMessageBox::warning(nullptr,"warning","There's no data in database for the specified pump!");
        return false;
    }
    ...
    {
        QMessageBox::warning(nullptr,"warning","Impeller trimed is out of range!");
        return false;
    }
...
void Pump_db::getLoadprofileList(QStringList* loadprofileAppList)
{
    if(!m_db.isOpen())
        return;
    ...
    sql = "select distinct application from " + m_tableName.loadprofileTableName;
    query.exec(sql);

    loadprofileAppList->clear();
    while(query.next())
        *loadprofileAppList<<query.value("application").toString();
}

bool Pump_db::getLoadprofile(QString loadprofileApp, QStandardItemModel *loadprofileData)
{
    if(!m_db.isOpen())
        return false;
    ...
    sql = "select * from " + m_tableName.loadprofileTableName +
            " where application = '" + loadprofileApp + "'" + " order by loading;";

    if(!query.exec(sql)) return false;

    loadprofileData->clear();
    while(query.next())
    {
        QList<QStandardItem*>   items;
        QStandardItem           *item;

        item = new QStandardItem(query.value("application").toString());
        items.append(item);
        item = new QStandardItem(query.value("loading").toString());
        items.append(item);
        item = new QStandardItem(query.value("hours").toString());
        items.append(item);
        loadprofileData->appendRow(items);
    }
    QStringList labels;
    labels.append("application");
    labels.append("loading");
    labels.append("hours");
    loadprofileData->setHorizontalHeaderLabels(labels);

    for(int i=0;i<2;i++)
    {
        QStandardItem *head_item = loadprofileData->horizontalHeaderItem(i);
        if(head_item)
        {
            head_item->setSelectable(false);
        }
   }
    for(int i=0;i<20;i++)
    {
        QStandardItem *table_item = loadprofileData->item(i,1);
        if(table_item)
        {
            table_item->setEditable(false);
            table_item->setBackground(QBrush(QColor(240,240,240)));
            table_item->setSelectable(false);
        }
    }
...
bool Pump_db::updateLoadprofile(QStandardItemModel *loadprofileData)
{
    if(!m_db.isOpen())
        return false;
    ...
    {
        sql1 = "delete from " + m_tableName.loadprofileTableName +
                " where application = '" + loadprofileData->item(i,0)->text() + "'" +
                " and loading = " + loadprofileData->item(i,1)->text() + ";";

        sql2 = "insert into " + m_tableName.loadprofileTableName + " values('" +
                loadprofileData->item(i,0)->text() + "'," + loadprofileData->item(i,1)->text() +
                "," + loadprofileData->item(i,2)->text() + ");" ;

        if(!query.exec(sql1)||!query.exec(sql2))
        {
            QMessageBox::warning(nullptr,"Warning","Update load profile failed at row "+QString::number(i));
            return false;
        }
    }
...
bool Pump_db::newLoadprofile(QString app)
{
    if(!m_db.isOpen())
        return false;
    ...
    {
        sql = "insert into " + m_tableName.loadprofileTableName +
                " values('" + app + "'," + QString::number(i) + ",0);";

        if(!query.exec(sql))
        {
            QMessageBox::warning(nullptr,"Warning","Create new load profile failed!");
            return false;
        }
    }
...
bool Pump_db::deleteLoadprofile(QString app)
{
    if(!m_db.isOpen())
        return false;
    ...
    sql = "delete from " + m_tableName.loadprofileTableName +
                " where application = '" + app + "';";

    if(!query.exec(sql))
    {
        QMessageBox::warning(nullptr,"","Delete load profile failed!");
        return false;
    }
    return true;
}
