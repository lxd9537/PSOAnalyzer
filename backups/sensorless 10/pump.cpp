...
bool Pump::WritePerformance(RawPerformData perform_data, int* error_code,QString* error_message)
{
    ...
    polyfit(perform_data.flow, perform_data.head, 5, CURVE_FIT_ORDER, m_perform_data.Kfactor_head_from_flow);
    polyfit(perform_data.flow, perform_data.power, 5, CURVE_FIT_ORDER, m_perform_data.Kfactor_power_from_flow);
    ...
    for(int i=0; i<5; i++)
    {
        ...
        m_perform_data.head_max = qMax(m_perform_data.head_max, perform_data.head[i]);
        m_perform_data.power_min = qMin(m_perform_data.power_min, perform_data.power[i]);
        m_perform_data.power_max = qMax(m_perform_data.power_max, perform_data.power[i]);
   }
...
void Pump::WritePerformance_2(PerformData perform_data)
{
    m_perform_data = perform_data;
}
...
double Pump::GetHeadFromFlow(double Hz, double flow, bool* EOC_flag)
{
    ...
    {
        Kfactor_head_from_flow[i] = m_perform_data.Kfactor_head_from_flow[i] * qPow(relative_speed, 2-i);
    }
...
double Pump::GetPowerFromFlow(double Hz, double flow, bool* EOC_flag)
{
    ...
    for(int i=0;i<CURVE_FIT_ORDER+1;i++)
    {
        Kfactor_power_from_flow[i] = m_perform_data.Kfactor_power_from_flow[i] * qPow(relative_speed, 3-i);
    }
...
Pump::MotorData Pump::ReadMotorData()
{
    return m_motorData;
}
void Pump::WriteMotorData(MotorData motorData)
{
    m_motorData = motorData;
}
double Pump::GetMotorEffi(double power, double speed) //speed: 0-1
{
    double effi = 0;
    double powerConverted;
    if(speed >0)
        powerConverted = power / pow(speed,3);
    else {
        powerConverted = power;
    }

    for(int i=0; i<CURVE_FIT_ORDER+1;i++)
        effi += m_motorData.Kfactor_effi_from_power[i]*pow(powerConverted,i);
