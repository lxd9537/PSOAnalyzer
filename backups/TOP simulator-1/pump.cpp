...
void Pump::Run_cycle(int period_ms,bool lack_water,int speed_up)
{

    //pump head/flow/power
    if(m_ro_data.running && !lack_water)
    {
    ...
    {
        m_ro_data.Hz += m_perform_data.Hz_max/m_perform_data.vfd_ascending_time * period_ms /1000 * speed_up;
        if (m_ro_data.Hz > required_Hz) m_ro_data.Hz = required_Hz;
    }
    else if (m_ro_data.Hz > required_Hz)
    {
        m_ro_data.Hz -= m_perform_data.Hz_max/m_perform_data.vfd_descending_time * period_ms /1000 * speed_up;
        if (m_ro_data.Hz < required_Hz) m_ro_data.Hz = required_Hz;
    }
